...
We can use the `rankVal` function to create a helper function for
pretty printing the rank:
...
Although we are operating at the type level, our `Length` function
looks very similar to the classic value-level length definition:
...
Now let's create a deck of cards. Writing out all 52 cards by hand
would be tedious, so lets take the cartesian product instead. First we
need an type-level `Append` function:
...
some subset of the deck. Note that although we did not explicitly
limit the number of the cards in the deck to being 52 or less, we got
